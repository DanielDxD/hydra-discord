...
import { formatUploadDate } from "../utils/helpers";
...
const getTorrentDetails = async (path: string) => {
    ...
    return {
        magnet: $a?.href,
        fileSize: ($totalSize.querySelector("span") as Element).textContent ?? undefined,
        uploadDate: formatUploadDate(
            ($dateUploaded.querySelector("span") as Element).textContent!
        ),
    };
};
...
export const getNewRepacksFromUser = async (
    ...
    if (!newRepacks.length) return null;
